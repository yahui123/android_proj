...
public class MyYWSDKGlobalConfig extends YWSDKGlobalConfig {
    ...
    //用于给主程序层拿配置
    public volatile static MyYWSDKGlobalConfig mInstance =null;

    public static MyYWSDKGlobalConfig getInstance(){
        if (mInstance == null) {
            synchronized(MyYWSDKGlobalConfig.class) {
                if (mInstance == null) {
                    mInstance = new MyYWSDKGlobalConfig(null);
                }
            }
        }
        return mInstance;
